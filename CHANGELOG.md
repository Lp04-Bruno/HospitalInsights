...
- Das Löschen eines Jahres in der Hospitalverwaltung lief in drei getrennten `deleteMany`-Aufrufen ohne Transaktion: Audit-Runs, Werte und Jahreszuordnung. Brach einer davon ab, blieb ein Zustand zurück, in dem die Audit-Belege bereits fehlten, die zugehörigen Werte aber noch vorhanden waren. Die Löschung läuft jetzt als eine Transaktion in `deleteHospitalPeriod`, und ein Fehlschlag meldet sich als Hinweis, statt in der Error Boundary zu landen.
